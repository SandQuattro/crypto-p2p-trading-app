...
// EnsureSufficientBNB проверяет и при необходимости пополняет баланс BNB на кошельке
func (bsc *WalletService) EnsureSufficientBNB(ctx context.Context, walletAddress string) error {
	...
	// Минимальный баланс BNB для оплаты газа (0.005 BNB)
	minBalance := new(big.Int).Mul(big.NewInt(5000000), big.NewInt(1000000000))

	// Если баланс меньше минимального, пополняем
	if balance.Cmp(minBalance) < 0 {
		// Получаем мастер-кошелек для отправки BNB
		masterWallet, err := bsc.GetMasterWallet(ctx)
		if err != nil {
			return fmt.Errorf("failed to get master wallet: %w", err)
		}

		// Отправляем 0.005 BNB на кошелек
		txHash, err := bsc.TransferBNB(ctx, masterWallet, walletAddress, minBalance)
		if err != nil {
			return fmt.Errorf("failed to transfer BNB: %w", err)
		}

		bsc.logger.Info("Sent BNB to wallet for gas",
...
// GetMasterWallet возвращает адрес мастер-кошелька для отправки BNB
func (bsc *WalletService) GetMasterWallet(_ context.Context) (string, error) {
	// Используем фиксированный индекс для мастер-кошелька
	childKeyIndex := uint32(0)
	childKey, err := bsc.masterKey.NewChildKey(childKeyIndex)
	if err != nil {
		return "", fmt.Errorf("failed to create child key: %w", err)
	}
...
// TransferBNB отправляет BNB с мастер-кошелька на указанный адрес
func (bsc *WalletService) TransferBNB(ctx context.Context, fromAddress, toAddress string, amount *big.Int) (string, error) {
	if bsc.masterKey == nil {
		return "", errors.New("master key not initialized")
	}

	// Получаем приватный ключ мастер-кошелька
	childKeyIndex := uint32(0)
	childKey, err := bsc.masterKey.NewChildKey(childKeyIndex)
	if err != nil {
		return "", fmt.Errorf("failed to create child key: %w", err)
	...
	gasLimit := uint64(21000) // Стандартный лимит газа для передачи BNB
